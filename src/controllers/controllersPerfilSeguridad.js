...
const createPerfilSeguridad = async (req, res, next) => {
  ...
  try {
    const perfilExist = await pool.query(
      `SELECT * FROM "perfilesSeguridad" WHERE "nombrePerfil" = $1`,
      [nombrePerfil]
    );

    if (perfilExist.rows.length > 0) {
      const updatePerfil = await pool.query(
        `UPDATE "perfilesSeguridad" SET "nombrePerfil" = $1 WHERE "idPerfilSeguridad" = $2 RETURNING *`,
        [nombrePerfil, perfilExist.rows[0].idPerfilSeguridad]
      );
      res.json(updatePerfil.rows[0]);
    } else {
      const newPerfil = await pool.query(
        `INSERT INTO "perfilesSeguridad" ("nombrePerfil") VALUES ($1) RETURNING *`,
        [nombrePerfil]
      );
...
const createPermisos_PerfilSeguridad = async (req, res, next) => {
  ...
  const permisosAlteradosArray = Object.entries(permisos).map(
    ([permiso, value]) => {
  ...
  if (permisosExist.rows.length > 0) {
    try {
      for (let i = 0; i < permisosAlteradosArray.length; i++) {
        const newPerfil = await pool.query(
          `UPDATE "permisos_Accesso" SET "${permisosAlteradosArray[i][0]}" = $1 WHERE "security_profile_id" = $2`,
          [permisosAlteradosArray[i][1], idPerfilSeguridad]
        );
        res.json(newPerfil.rows[0]);
      }
    } catch (error) {
    ...
    try {
      await pool.query(
        `INSERT INTO "permisos_Accesso" ("security_profile_id") VALUES ($1) RETURNING *`,
        [idPerfilSeguridad]
      );
...
const deshabilitarPerfilSeguridad = async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(200)
      .json(
        `El perfil de seguridad con el id ${idPerfilSeguridad} ha sido deshabilitado`
      );
  } catch (error) {
    console.log(error.message);
  }
